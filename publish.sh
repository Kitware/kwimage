...
#DEFAULT_MODE_LIST=("sdist" "native" "bdist")
...
if [[ "$DO_UPLOAD" == "True" ]]; then
    ...
    do
        if [ "$DO_GPG" == "True" ]; then
            twine upload --username $TWINE_USERNAME --password=$TWINE_PASSWORD $WHEEL_PATH.asc $WHEEL_PATH  || { echo 'failed to twine upload' ; exit 1; }
        else
