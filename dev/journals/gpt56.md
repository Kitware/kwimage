...
## 2026-09-20 17:55:00 -0400
Worked on integrating the Rust-first `kwimage_ext` NMS backend into kwimage and fixing minimal CI collection when `kwconf` is absent. The important dispatch bug was that kwimage treated importability of `kwimage_ext.algo._nms_backend.gpu_nms` as proof of a usable CUDA backend; current Rust-first kwimage_ext intentionally provides a Python compatibility stub at that path that raises `NotImplementedError`. On CUDA-capable hosts the old heuristic could therefore advertise and auto-select a backend that cannot run.

I chose to make Rust CPU NMS a canonical `rust_cpu` backend when kwimage_ext reports Rust metadata, while preserving `cython_cpu` as an explicit compatibility alias. The historical compiled `cython_gpu` backend remains usable when a native extension is genuinely installed, but automatic selection no longer chooses it. For CUDA tensors, torchvision remains preferred when available to avoid an unnecessary device-to-host transfer; for NumPy and CPU inputs Rust CPU is preferred. This keeps compatibility without encoding the obsolete GPU stub as capability.

The minimal test failure is intentionally handled at the kwconf-specific test module with `pytest.importorskip`, because kwconf is an optional dependency and the minimal CI environment should not be forced to install it solely for those CLI integration tests. I did not change generated CI plumbing. Risks to watch are downstream callers that inspect the exact list/order from `available_nms_impls`, and performance of automatic selection for CUDA tensors when torchvision is unavailable; the legacy GPU implementation is still explicitly requestable if a real compiled module exists.
