...
def _bench_index_select_put():
    ...
    for timer in ti.reset('select multi-index (torch.index_select)'):
        with timer:
            ...
            select_result1 = torch.index_select(
                class_energy, dim=dim, index=index
            )
...
def _bench_catgraph_conditional_log_softmax_solution():
    ...
    def method1(self, class_energy, dim):
        ...
        for index in index_groups:
            ...
            energy_group = torch.index_select(
                class_energy, dim=dim, index=index
            )
    ...
    def method2(self, class_energy, dim):
        ...
        for idxs in self.idx_groups:
            fancy_index = tuple(fancy_prefix + [idxs])
            cond_logits[fancy_index] = F.log_softmax(
...
def _bench_catgraph_sink_log_softmax():
    ...
    def sink_log_softmax_method1(self, class_energy, dim):
        leaf_idxs = sorted(
            self.node_to_idx[node] for node in sink_nodes(self.graph)
        )
        class_logits = torch.empty_like(class_energy)

        fancy_prefix = [slice(None)] * dim
        fancy_index = tuple(fancy_prefix + [leaf_idxs])
        class_logits[fancy_index] = F.log_softmax(
            class_energy[fancy_index], dim=dim
        )
        ...
        def populate1(node):
            ...
            if len(children) > 0:
                ...
                class_logits[fancy_node_index] = torch.logsumexp(
                    class_logits[fancy_children_index], dim=dim
                )
    ...
    def sink_log_softmax_method2(self, class_energy, dim):
        ...
        leaf_energy = torch.index_select(class_energy, dim=dim, index=leaf_idxs)
        ...
        def populate2(node):
            ...
            if len(children) > 0:
                ...
                selected = torch.index_select(
                    class_logits, dim=dim, index=child_idxs
                )
