...
//  https://raw.githubusercontent.com/martin-steinegger/setcover/master/SetCover.cpp
...
{
    set_cover s_algo(set_count, uniqu_element_count, max_weight, all_element_count,
        element_size_lookup);

    for(unsigned int i = 0; i < set_count; i++)
    {
        s_algo.add_set(i+1, set_sizes[i],
                          (const unsigned int*)sets[i],
                          (const unsigned short*)weights[i],
                          set_sizes[i]);
    }

    std::list<set *> ret= s_algo.execute_set_cover();
    std::list<set *>::const_iterator iterator;
    unsigned int j = 0;
    for (iterator = ret.begin(); iterator != ret.end(); ++iterator) {
        outvar_keep[j++] = (*iterator)->set_id;
    }
}
