...
class _HeatmapDrawMixin(object):
    ...
    def _colorize_class_idx(self) -> ndarray:
        ...
        if index_is_negative.any():
            ...
            if special_ignore_classname not in name_to_color:
                ...
                special_ignore_color = kwimage.Color.coerce(
                    'kitware_gray'
                ).as01()
    ...
    def colorize(
        ...
        def _per_channel_color(
            data: Any, with_alpha: float, classes: Any | None = None
        ) -> ndarray:
            # Another hacky mode
    ...
    def draw_stacked(
        ...
        if image is not None:
            if mat is not None:
                ...
                dataspace_img = cv2.warpAffine(
                    image, mat[0:2], dsize=level_dsize
                )
            else:
                dataspace_img = image
        ...
        for cx in chosen_cxs:
            ...
            c = kwimage.draw_text_on_image(
                c, '{}'.format(node), (0, 20), fontScale=0.5
            )
    ...
    def draw_on(
        ...
        if imgspace is None:
            imgspace = bool(np.all(image.shape[0:2] == np.array(self.img_dims)))
...
class _HeatmapWarpMixin(object):
    ...
    def _align_other(self, other: Heatmap) -> Heatmap:
        """
    ...
    def _align(self, mask: Any, interpolation: str = 'linear') -> ndarray:
        """
    ...
    def warp(
        ...
        newself = cast(Heatmap, cls(newdata, newmeta))
        return newself
...
class _HeatmapAlgoMixin(object):
    ...
    def combine(
        ...
        if (
            'keypoints' in aligned_root.data
            and aligned_root.data['keypoints'] is not None
        ):
            newdata['keypoints'] = amean(
                [h.data['keypoints'] for h in aligned_heatmaps]
            )
    ...
    def detect(
        ...
        max_dims_arr: ndarray | None = None
        if max_dims is not None:
            if ub.iterable(max_dims):
                max_dims_arr = np.asarray(cast(Any, max_dims))
            else:
                max_dims_arr = np.asarray((max_dims, max_dims))

        min_dims_arr: ndarray | None = None
        if min_dims is not None:
            if ub.iterable(min_dims):
                min_dims_arr = np.asarray(cast(Any, min_dims))
            else:
                min_dims_arr = np.asarray((min_dims, min_dims))
        ...
        if dim_thresh_space == 'image':
            ...
            if max_dims_arr is not None:
                max_dims_arr = max_dims_arr / scale_dims
            if min_dims_arr is not None:
                min_dims_arr = min_dims_arr / scale_dims
        ...
        dets = _prob_to_dets(
            ...
            min_score=min_score,
            num_min=num_min,
            max_dims=max_dims_arr,
            min_dims=min_dims_arr,
        )
...
class Heatmap(
    _generic.Spatial, _HeatmapDrawMixin, _HeatmapWarpMixin, _HeatmapAlgoMixin
):
    """
    ...
    def random(
        ...
        if dets == 'coco':
            ...
            dets = kwimage.Detections.from_coco_annots(
                anns,
                cats=sampler.dset.dataset['categories'],
                shape=input_dims,
        ...
        if img_dims is None:
            ...
            wh_dims = dims[::-1]
            img_wh_dims = tuple(
                np.ceil(tf_data_to_img([wh_dims]))[0].astype(int).tolist()
            )
            img_dims = img_wh_dims[::-1]
        else:
            img_dims_arr = np.asarray(img_dims)
            tf_data_to_img = skimage.transform.AffineTransform(
                scale=(img_dims_arr / np.asarray(dims))[::-1],
                translation=(0, 0),
            )
        ...
        class_probs = np.array(
            [smooth_prob(p, k=smooth_k) for p in class_probs]
        )
        ...
        if keypoints is not False and keypoints is not None:
            ...
            if 'kp_classes' not in locals():
                kp_classes = list(
                    range(self.data['keypoints'].shape[1])
                )  # HACK
...
def _prob_to_dets(
    ...
    diameter_is_uniform = tuple(getattr(diameter, 'shape', []))[1:] != tuple(
        probs.shape
    )
    offset_is_uniform = tuple(getattr(offset, 'shape', []))[1:] != tuple(
        probs.shape
    )
    diameter_data = cast(Any, diameter)
    offset_data = cast(Any, offset)

    if diameter_is_uniform:
        if hasattr(diameter_data, 'shape'):
            if len(diameter_data.shape) > 2:
                raise Exception(
                    'Trailing diameter shape={} does not agree with probs.shape={}'.format(
                        diameter_data.shape, probs.shape
                    )
                )

        if not ub.iterable(diameter_data):
            diameter_data = [diameter_data, diameter_data]

    if offset_is_uniform:
        if not ub.iterable(offset_data):
            offset_data = impl.asarray([offset_data, offset_data])
    ...
    if not diameter_is_uniform:
        if max_dims is not None:
            ...
            if max_height is not None:
                flags &= diameter_data[0] <= max_height
            if max_width is not None:
                flags &= diameter_data[1] <= max_width
        if min_dims is not None:
            min_dims = (
            ...
            if min_height is not None:
                flags &= diameter_data[0] >= min_height
            if min_width is not None:
                flags &= diameter_data[1] >= min_width
    ...
    if num_min is not None:
        ...
        if flags.sum() < num_min:
            if impl.is_tensor:
                topxs = probs.view(-1).argsort()[
                    max(0, numel - num_min) : numel
                ]
    ...
    if diameter_is_uniform:
        h = impl.full_like(yc_, fill_value=diameter_data[0])
        w = impl.full_like(xc_, fill_value=diameter_data[1])
    else:
        h = impl.astype(diameter_data[0][flags], np.float32)
        w = impl.astype(diameter_data[1][flags], np.float32)
    cxywh = impl.cat(
        [xc_[:, None], yc_[:, None], w[:, None], h[:, None]], axis=1
    )
    ...
    if offset is not None:
        if offset_is_uniform:
            det_dxdy = offset_data[[1, 0]]
        else:
            det_dxdy = impl.T(offset_data[:, yc, xc][[1, 0]])
    ...
    if keypoints is not None:
        ...
        if 1:
            ...
            det_coords = [kwimage.Coords(xys) for xys in det_kpts_xy]
...
def smooth_prob(
    prob: ndarray, k: int = 3, inplace: bool = False, eps: float = 1e-9
) -> ndarray:
    """
    ...
    if prob.size == 0:
        ...
        # NumPy's scalar argmin/argmax overload is not modeled consistently
        # across supported type-checker / NumPy stub combinations.
        min_flat = int(cast(Any, blur).argmin())
        max_flat = int(cast(Any, blur).argmax())
        minpos = np.unravel_index(min_flat, blur.shape)
        maxpos = np.unravel_index(max_flat, blur.shape)
...
def _remove_translation(tf: Any) -> Any:
    """
